...
cat $1 | awk -F'\t' '
    NR>1 {
        ...
        if($15 == "NA") {
            printf "INSERT IGNORE INTO signature (signatureName, cellTypeA, cellTypeB, sourceDb, experimentalDesign, organism, disease) VALUES (\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\");\n", $1, $4, $5, $3, toupper($8), $6, $7;
        } else {
